Shader "TestShaders/DynamicCloud"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float cloud(float2 uv)
            {
                ...
                float ol = length(o);
                o += 0.05 * float2(_CloudSpeed * 1.35 *_Time.x + ol,
                    _CloudSpeed * 1.5 * _Time.x + ol);
                o *= 2;
                float2 n = float2(fbm(o + 9, 6), fbm(o + 5, 6));
                float f = fbm(2 * (uv + n), 4);
                f = f * 0.5 + smoothstep(0, 1, pow(f, 3) * pow(n.x, 2)) * 0.5 +
                    smoothstep(0, 1, pow(f, 5) * pow(n.y, 2)) * 0.5;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float2 uv = float2((atan2(viewDir.x, viewDir.z) + UNITY_PI) / 
                    UNITY_TWO_PI, acos(viewDir.y) / UNITY_PI);
                uv = uv * _MainTex_ST.xy + _MainTex_ST.zw;
                fixed4 tex = tex2D(_MainTex, uv, float2(0.00001, 0.0), 
                    float2(0.0, 0.0f));
                ...
                float2 _uv = float2(sin(th) * 0.5, cos(th) * 0.5) * s * 5 * 
                    _CloudNumber + 0.5;
